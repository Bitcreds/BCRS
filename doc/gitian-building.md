...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bitcreds-host/README.md).
...
- Storage on Physical hard drive: Bitcredsally Allocated 
...
Clone the git repositories for bitcreds and gitian and then checkout the bitcreds version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bitcreds repository
...
To build Bitcreds (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitcreds repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcreds/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bitcreds-crds/bitcreds
...
./bin/gbuild --commit bitcreds=${COMMIT} --url bitcreds=${URL} ../bitcreds/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcreds=${COMMIT} --url bitcreds=${URL} ../bitcreds/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcreds=${COMMIT} --url bitcreds=${URL} ../bitcreds/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcreds-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcreds-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bitcreds-build.assert
```
