...
bool CMasternodeBroadcast::Create(CTxIn txin, CService service, CKey keyCollateralAddressNew, CPubKey pubKeyCollateralAddressNew, CKey keyMasternodeNew, CPubKey pubKeyMasternodeNew, std::string &strErrorRet, CMasternodeBroadcast &mnbRet)
{
    ...
    LogPrint("Masternode", "CMasternodeBroadcast::Create -- pubKeyCollateralAddressNew = %s, pubKeyMasternodeNew.GetID() = %s\n",
             CBitcredsAddress(pubKeyCollateralAddressNew.GetID()).ToString(),
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if (coins.vout[vin.prevout.n].nValue != Params().GetConsensus().GetCurrentMasternodeCollateral(chainActive.Height()) * COIN) {
            LogPrint("Masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %d BCRS, Masternode=%s\n", Params().GetConsensus().GetCurrentMasternodeCollateral(chainActive.Height()), vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 10000 BCRS tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 10000 BCRS tx -> 1 confirmation
...
bool CMasternodeBroadcast::IsVinAssociatedWithPubkey(const CTxIn& txin, const CPubKey& pubkey)
{
    ...
    if(GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if (out.nValue == Params().GetConsensus().GetCurrentMasternodeCollateral(chainActive.Height()) * COIN && out.scriptPubKey == payee)
                return true;
    }
