...
void ImportAddress(const CBitcredsAddress& address, const std::string& strLabel);
void ImportScript(const CScript& script, const std::string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CBitcredsAddress(CScriptID(script)), strLabel);
    }
}
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CBitcredsAddress(pubKey.GetID()), strLabel);
...
UniValue importwallet(const UniValue& params, bool fHelp)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CBitcredsAddress(keyid).ToString());
            continue;
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "dumpprivkey \"bitcredsaddress\"\n"
            "\nReveals the private key corresponding to 'bitcredsaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"bitcredsaddress\"   (string, required) The bitcreds address for the private key\n"
            "\nResult:\n"
    ...
    return CBitcredsSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->GetHDChain(hdChainCurrent))
    {
        ...
        CBitcredsExtPubKey b58extpubkey;
...
for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CBitcredsAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CBitcredsSecret(key).ToString(), strTime);
