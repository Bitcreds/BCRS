    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    QString version = tr("Bitcreds") + " " + tr("version") + " " + QString::fromStdString(FormatFullVersion());
    ...
    if (helpMode == about)
    {
        setWindowTitle(tr("About Bitcreds"));
        ...
        std::string strUsage = HelpMessage(HMM_BITCREDS_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BitcredsGUI::DEFAULT_UIPLATFORM));
        }
...
These denominations are 0.01 BCRS, 0.1 BCRS, 1 BCRS, 10 BCRS -- sort of like the paper money you use every day.</li> \
    ...
    QWidget(parent, f)
{
    ...
    layout->addWidget(new QLabel(
        tr("Bitcreds is shutting down...") + "<br /><br />" +
        tr("Do not shut down the computer until this window disappears.")));
    setLayout(layout);
}

void ShutdownWindow::showShutdownWindow(BitcredsGUI *window)
{
    if (!window)
        return;
